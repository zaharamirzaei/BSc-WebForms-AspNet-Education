    ...
    protected void uploadbtn_Click(object sender, EventArgs e)
    {
        if ((imgUpload.HasFile) && (FileUpload.HasFile))
        {
            if ((checktype1(imgUpload.FileName)) && (checktype2(FileUpload.FileName)))
            {
                ...
                pta.InsertPost(titlepost.Text, decpost.Text, 0, filepost, dt, postimg, int.Parse(catid_txt.SelectedValue),false,Session["useremail"].ToString());

                imgUpload.SaveAs(MapPath("~/img/" + imgUpload.FileName));
                FileUpload.SaveAs(MapPath("~/files/" + FileUpload.FileName));
                titlepost.Text = "";
                decpost.Text = "";
                Response.Write("<script>alert('فایل با موفقیت اپلود شد و پس از تایید در سایت قرار خواهد گرفت')</script>");
